...
class TinyHabitsExporter(ExporterXL):
    def __init__(self, config: dict) -> None:
        super().__init__(config)


if __name__ == "__main__":
    title = "tiny habits exporter"
    version = "0.1.0"
    ...
    config = {
        ...
        "credential": os.path.abspath(
            os.path.join(agents_dir, "haru-tiny-habits.json")
        ),
        "export_directory": exports_dir,
        "export_filename": "Haru-Tiny-Habits.xlsx",
        "algorithm": "dfs",
        "mode": "text_rr",
    }

    exporter = TinyHabitsExporter(config)
    exporter.run()
